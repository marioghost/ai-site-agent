...
def test_reasoning_service_passthrough_preserves_answer_and_sources(monkeypatch):
    ...
    # Scoped overview with grounded sources → sufficient (no completeness-risk footer)
    assert wrapped.evidence_sufficient is True
    assert wrapped.sufficiency is not None
    assert wrapped.sufficiency.completeness_risk is False
    ...
    assert wrapped.speech_act_decision.qualification_required is False
