...
def score_content_match(
    *,
    ...
    body = text or ""
    head_preview = body[: min(_MAIN_HEAD_CHARS, len(body))]
    main_preview = body[: min(_MAIN_PREVIEW_CHARS, len(body))]
    tail_preview = body[len(main_preview) :]
    head_tokens = token_set(head_preview)
    ...
    head_hits = query_tokens & head_tokens
    ...
    main_score = 0.0
    if main_hits:
        head_ratio = len(head_hits) / max(1, len(query_tokens))
        main_ratio = len(main_hits) / max(1, len(query_tokens))
        main_score = (
            min(title_boost, 0.12) * head_ratio
            + min(0.12, 0.08 * main_ratio)
        )
        if main_hits and not head_hits:
            main_score += 0.03
    if title_hits or heading_hits:
        main_score += min(0.08, 0.04 * len(title_hits | heading_hits))
    if tail_hits and not main_hits:
        main_score += min(0.05, 0.02 * len(tail_hits))

    url_score = 0.06 if url_hits else 0.0

    nav_penalty = 0.0
    reason = ""
    if full_hits and not (title_hits or heading_hits or head_hits or url_hits):
        nav_penalty = _NAV_ONLY_PENALTY
        reason = "query_terms_nav_only"
    elif tail_hits and not (title_hits or heading_hits or head_hits or url_hits):
