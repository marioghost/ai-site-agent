#!/usr/bin/env bash
# Backend unit test suite (RFC migration gate). No Postgres required for unit tests.
set -euo pipefail
...
echo "==> Backend unit tests (RFC migration suite)"
"$VENV/bin/pytest" \
...
echo "OK: backend unit suite passed"
