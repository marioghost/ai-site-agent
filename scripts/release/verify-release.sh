#!/usr/bin/env bash
# Release verification — single human-readable report.
...
if [[ -n "$DEPLOYED_COMMIT" && "$DEPLOYED_COMMIT" == "$ORIGIN_MAIN" ]]; then
  ok "build-info == origin/main"
else
  bad "build-info ($DEPLOYED_COMMIT) != origin/main ($ORIGIN_MAIN)"
fi
if [[ -f "$FE_ID" ]]; then
...
if curl -sf --max-time 5 "$BUILD_URL" -o /tmp/vr-build.json 2>/dev/null; then
  ...
  if [[ -n "$API_COMMIT" && "$API_COMMIT" == "$ORIGIN_MAIN" ]]; then
    ok "/api/build matches origin/main"
  else
    bad "/api/build ($API_COMMIT) != origin/main ($ORIGIN_MAIN)"
  fi
  # Feature flags — report only; do not fail on OFF (expected for 0.7)
...
if curl -sf --max-time 5 "$OVERVIEW_URL" -o /tmp/vr-overview.json 2>/dev/null; then
  python3 - <<'PY' 2>/dev/null | sed 's/^/  /' || true
import json
d=json.load(open("/tmp/vr-overview.json"))
# Best-effort keys — overview shape may vary
for k in ("sources_count","chunks_count","claims_count","qdrant_points","knowledge_version","memory_version"):
    if k in d: print(f"{k}={d[k]}")
# nested
for path in (("stats",), ("corpus",), ("counts",)):
    cur=d
    okp=True
    for p in path:
        if isinstance(cur, dict) and p in cur: cur=cur[p]
        else: okp=False; break
    if okp and isinstance(cur, dict):
        for k,v in list(cur.items())[:12]:
            print(f"{'.'.join(path)}.{k}={v}")
PY
  ok "overview reachable (corpus snapshot printed if present)"
else
...
if curl -sf --max-time 3 "http://127.0.0.1:6333/collections" -o /tmp/vr-qdrant.json 2>/dev/null; then
  python3 - <<'PY' 2>/dev/null | sed 's/^/  /' || true
import json
d=json.load(open("/tmp/vr-qdrant.json"))
cols=d.get("result",{}).get("collections") or d.get("collections") or []
for c in cols[:10]:
...
if [[ -n "$ORIGIN_MAIN" && "$ORIGIN_MAIN" == "$DEPLOYED_COMMIT" && "$DEPLOYED_COMMIT" == "${API_COMMIT:-}" && "$DEPLOYED_COMMIT" == "${FE_COMMIT:-}" ]]; then
  ok "FULL CHAIN ALIGNED (origin/main == build-info == frontend == /api/build)"
else
