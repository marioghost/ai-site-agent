...
import SourceDetailDrawer from "./widgets/SourceDetailDrawer";
import SourcesBulkBar from "./widgets/SourcesBulkBar";
import SourcesFilters, {
  EMPTY_FILTERS,
  type SourceFilterValues,
} from "./widgets/SourcesFilters";
import SourcesHeader from "./widgets/SourcesHeader";
import SourcesIndexingBanner from "./widgets/SourcesIndexingBanner";
import SourcesKnowledgeMiniCard from "./widgets/SourcesKnowledgeMiniCard";
import SourcesPagination from "./widgets/SourcesPagination";
import SourcesSummaryCards from "./widgets/SourcesSummaryCards";
import SourcesTable, { type MenuAction } from "./widgets/SourcesTable";
...
export default function LibraryScreen() {
  ...
  const runBulk = async (fn: (ids: number[]) => Promise<unknown>) => {
    ...
    try {
      ...
      clearSelection();
      await refreshAll();
      if (activeId != null && ids.includes(activeId)) closeDetail();
    } finally {
      setBusy(false);
    }
  };
  ...
  const onIndexAllPending = async () => {
    ...
    try {
      ...
      if (latest?.status === "failed") {
        setIndexError(indexJobErrorMessage(latest) || t("sources.indexing_failed"));
  ...
  return (
    <PageLayout className="ds-page--wide">
      ...
      {knowledgeBase && <SourcesKnowledgeMiniCard data={knowledgeBase} lang={lang} t={t} />}
    </PageLayout>
  );
}
